...
const FIRST_TASK = Object.freeze({
  ...
  prompt: '前の人が残した写真を見て、次の人に残したいと思う部分を1つタップしてください。',
  hint: '「なぜか目に入る」場所で十分です。説明はまだいりません。',
  actionLabel: ACTION_META[ACTIONS.CHOOSE].label,
  step: 1,
  remaining: MAX_STEPS,
});
...
function normalizeAction(action) { return ACTION_META[action] ? action : null; }
...
export function nextTaskForStep(step) { return step === 0 ? FIRST_TASK : null; }
...
export function selectNextAction(previous, history = [], seed = '') {
  ...
  const candidates = (TRANSITIONS[current] || [ACTIONS.CHOOSE]).filter((action) => !history.slice(-2).includes(action));
...
export function generateNextTask({ previous, history = [], seed = '', step = 1 }) {
  ...
  const nextAction = step === 1 ? ACTIONS.CHOOSE : selectNextAction(normalizedPrevious, history, seed);
  const nextTask = taskForAction(nextAction, { previous: normalizedPrevious });
  return { ...nextTask, step, remaining: Math.max(0, MAX_STEPS - step + 1) };
}
