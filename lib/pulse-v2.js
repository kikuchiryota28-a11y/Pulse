...
const META = Object.freeze({
  CAPTURE: { label: '撮る', inputType: 'photo' },
  FIND: { label: '探す', inputType: 'photo' },
  CHOOSE: { label: '選ぶ', inputType: 'tap' },
  INTERPRET: { label: '解釈する', inputType: 'text' },
  COMPARE: { label: '比べる', inputType: 'compare' },
  CHALLENGE: { label: '疑う', inputType: 'challenge' },
...
export function taskForAction(actionType, { previous } = {}) {
  ...
  const tasks = {
    CHOOSE: [
      'この写真の中で、次の人に残したいと思う部分を1つタップしてください。',
      '写真の中で、なぜか目に入る場所を1つ選んでください。',
    ],
    INTERPRET: [
      '前の人が選んだ場所は、なぜ気になるのでしょう？あなたの仮説を一文で書いてください。',
      'ここまでの行動に、どんな意味が生まれたと思いますか？一文で書いてください。',
    ],
    FIND: [
      `「${clip(context || 'この見方')}」を裏付けるものを、今いる場所から1つ探して撮ってください。`,
      'ここまでの流れを証明できそうなものを、今いる場所から1つ探して撮ってください。',
    ],
    CHALLENGE: [
      '前の人の主張を少しだけ疑ってください。反証になりそうなものを1つ撮り、その理由を短く書いてください。',
    ],
    COMPARE: [
      '前の人が残したものと、対比できる別のものを1つ撮ってください。その違いを短く書いてください。',
    ],
    PREDICT: [
      'ここまでの流れを見て、次の人は何を見つけそうですか？一文で予測してください。',
    ],
    CAPTURE: [
      'ここまでの流れを受けて、あなたなら残したい一枚を撮ってください。',
    ],
  };
  const titles = {
    CHOOSE: 'この写真の中で、気になる場所を選ぶ。',
    INTERPRET: '意味を与える。',
    FIND: 'その仮説を、現実で探す。',
    CHALLENGE: '本当にそうなのか、疑う。',
    COMPARE: '違うものを並べる。',
    PREDICT: '次に起きそうなことを予測する。',
    CAPTURE: '次の一手を残す。',
  };
  const hints = {
    CHOOSE: '写真を1回タップしてください。',
    INTERPRET: '短い一文で十分です。',
    FIND: '人・危険な場所は避けてください。',
    CHALLENGE: '「本当に？」を起点に、別の証拠を探します。',
    COMPARE: '同じものではなく、違いが出るものを選びます。',
    PREDICT: '正解ではなく、次に起きそうなことを書きます。',
    CAPTURE: 'この流れに合う一枚を残してください。',
  };
  return {
    version: PULSE_V3,
    actionType: action,
    ...
    title: titles[action],
    prompt: stableChoice(tasks[action], `${context}:${action}`),
    hint: hints[action],
...
export function serializeStep({ artifact, action, result, step, task }) {
  return JSON.stringify({
    ...
    action,
    result,
    title: result?.text || result?.summary || null,
    step,
    task: task || null,
  });
}
