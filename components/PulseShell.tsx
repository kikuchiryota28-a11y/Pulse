...
export type PulseTab = "DISCOVER" | "EXPLORE" | "CREATE" | "YOU";
...
const tabs: Array<{ label: PulseTab; href: string }> = [
  { label: "DISCOVER", href: "/" },
...
function getActiveTab(pathname: string): PulseTab {
...
export default function PulseShell({ children, activeTab }: PulseShellProps) {
  ...
  return (
    <div className="pulse-shell relative min-h-screen overflow-x-hidden bg-[#08080A] text-white selection:bg-[#55FF9A]/20 selection:text-white">
      {/* Ambient layer: deliberately behind every interactive surface. */}
        ...
        <div className="mx-auto flex h-full max-w-[1600px] items-center justify-between px-5 md:px-8">
          <Link href="/" className="flex items-center gap-3" aria-label="Pulse home">
            <span className="relative flex h-2 w-2 items-center justify-center">
              <span className="absolute h-2 w-2 animate-ping rounded-full bg-[#55FF9A] shadow-[0_0_12px_rgba(85,255,154,0.9)]" />
              <span className="relative h-1.5 w-1.5 rounded-full bg-[#55FF9A]" />
            </span>
      ...
      <main className="relative z-10 min-h-screen px-5 pb-32 pt-24 md:px-8">
        <div className="mx-auto max-w-[1600px]">{children}</div>
      </main>

      <nav
        className="fixed bottom-5 left-1/2 z-50 flex -translate-x-1/2 items-center gap-1 rounded-full border border-white/[0.09] bg-[#111114]/75 px-2 py-2 backdrop-blur-2xl"
        ...
        {tabs.map(({ label, href }) => {
          ...
          return (
            <Link
              ...
              className={`group relative flex items-center gap-2 rounded-full px-4 py-2.5 font-mono text-[9px] tracking-[0.08em] transition-all duration-300 ${
                isActive ? "text-white" : "text-white/35 hover:text-white/75"
              }`}
            >
              {isActive && <span className="h-1.5 w-1.5 rounded-full bg-[#55FF9A] shadow-[0_0_10px_rgba(85,255,154,0.85)]" />}
              <span>{label}</span>
            </Link>
          );
        })}
