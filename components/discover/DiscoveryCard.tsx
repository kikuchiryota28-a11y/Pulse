...
export function DiscoveryCard({ post, index = 0, onOpen }: DiscoveryCardProps) {
  ...
  const [reactionCount, setReactionCount] = useState(12 + index * 7);

  const chooseReaction = (type: ReactionType) => {
    setReaction((current) => {
      if (current === type) { setReactionCount((count) => Math.max(0, count - 1)); return null; }
      if (!current) setReactionCount((count) => count + 1);
      return type;
    });
  };
      ...
      <div className="px-4 pb-4 pt-4 sm:px-5 sm:pb-5">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-white/[.06] pt-4">
          {reactions.map(({ type, label, icon: Icon }) => {
            ...
            return <button key={type} type="button" onClick={() => chooseReaction(type)} aria-pressed={active} className={`inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-xs transition ${active ? 'bg-white text-black' : 'bg-white/[.04] text-white/45 hover:bg-white/[.08] hover:text-white'}`}><Icon size={13} fill={active && type === 'loved' ? 'currentColor' : 'none'} />{label}</button>;
          })}
          <span className="ml-1 text-xs text-white/20">{reactionCount}</span>
          <button type="button" onClick={() => setSaved((value) => !value)} aria-pressed={saved} className={`ml-auto inline-flex items-center gap-1.5 rounded-full px-3 py-2 text-xs transition ${saved ? 'bg-white text-black' : 'bg-white/[.04] text-white/45 hover:bg-white/[.08] hover:text-white'}`}><Bookmark size={13} fill={saved ? 'currentColor' : 'none'} />{saved ? 'Saved' : '知の棚へ'}</button>
