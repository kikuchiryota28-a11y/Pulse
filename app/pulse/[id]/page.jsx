...
export default function PulseDeepLink({ params }) {
  ...
  useEffect(() => { setActor(actorId()); load(); setMoveOpen(searchParams.get('move') === '1'); }, [id, searchParams]);
  useEffect(() => {
    if (!pulse?.id) return;
  ...
  const submitMove = async () => {
    ...
    try {
      ...
      const { error: insertError } = await supabase.from('pulse_moves').insert({
        pulse_id: pulse.id, actor_id: actor, parent_move_id: current?.id || null,
        depth: (current?.depth || 0) + 1, action_type: director?.actionType || 'interpret', input_type:'text',
        prompt: director?.prompt || 'Change this in your own words.', content: compactContent(content), state_before: stateBefore,
      ...
      setDraft(''); setMoveOpen(false); await load();
    } catch(e) { setMoveError(e?.message || 'Could not save your move.'); }
  ...
  return <main className="min-h-screen bg-[#f4f1e9] text-[#20221d]"><div className="mx-auto max-w-[760px] px-4 pb-32 pt-4 sm:px-6">
    <div className="mb-4 flex items-center justify-between"><button onClick={() => router.back()} className="flex items-center gap-2 rounded-full border border-black/10 bg-white/80 px-3 py-2 text-xs font-semibold backdrop-blur"><ArrowLeft size={15}/> Back</button><div className="flex gap-2"><button onClick={like} aria-label="Like" className={`grid h-10 w-10 place-items-center rounded-full border border-black/10 bg-white ${liked?'text-red-500':''}`}><Heart size={17} fill={liked?'currentColor':'none'}/></button><button onClick={share} aria-label="Share" className="grid h-10 w-10 place-items-center rounded-full border border-black/10 bg-white"><Share2 size={17}/></button></div></div>
    <motion.article initial={{opacity:0,y:14}} animate={{opacity:1,y:0}} className="overflow-hidden rounded-[30px] bg-white shadow-[0_20px_70px_rgba(0,0,0,.07)]"><div className="aspect-[4/3] w-full"><Media src={currentMedia} alt={pulse.title}/></div><div className="p-6 sm:p-8"><div className="flex items-center justify-between text-[10px] font-bold uppercase tracking-[.14em] text-black/45"><span>{pulse.status==='active'?'STILL MOVING':'COMPLETED'}</span><span>{formatRelative(pulse.updated_at)}</span></div><h1 className="mt-3 text-[clamp(2rem,6vw,3.5rem)] font-semibold leading-[.96] tracking-[-.055em]">{pulse.title}</h1><p className="mt-4 text-sm leading-6 text-black/55">{pulse.intent || 'People are changing this post one move at a time.'}</p><div className="mt-5 flex flex-wrap items-center gap-4 text-xs font-semibold text-black/50"><span className="inline-flex items-center gap-1.5"><Users size={14}/>{participantCount(ordered)} changed</span><span className="inline-flex items-center gap-1.5"><Zap size={13}/>{ordered.length} moves</span></div></div></motion.article>
