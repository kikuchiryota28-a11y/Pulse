...
import { ArrowLeft, ArrowRight, Heart, Share2, Users, Zap, AlertTriangle, RefreshCw } from 'lucide-react';
import { useRouter } from 'next/navigation';
import { supabase } from '../../../lib/supabase';
import { actorId, buildState, contentFromMove, contentPreview, formatRelative, mediaFromContent, participantCount, seedFromPulse } from '../../../lib/pulse-social';
...
export default function PulseDeepLink({ params }) {
  ...
  const share = async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('pulse:shared'));
    } catch {
      window.prompt('Copy this Pulse link', url);
    }
  };
  ...
  const like = async () => {
    ...
    try {
      if (!liked) {
        await supabase.from('pulse_reactions').insert({ pulse_id: pulse.id, actor_id: actor, reaction: 'like' });
      } else {
        await supabase.from('pulse_reactions').delete().eq('pulse_id', pulse.id).eq('actor_id', actor).eq('reaction', 'like');
      }
    } catch {
      setLiked((v) => !v);
    }
  ...
  return <main className="min-h-screen bg-[#f4f1e9] text-[#20221d]">
    <div className="mx-auto max-w-[760px] px-4 pb-28 pt-4 sm:px-6">
      ...
      <motion.article initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} className="overflow-hidden rounded-[30px] bg-white shadow-[0_20px_70px_rgba(0,0,0,.07)]">
        ...
        <div className="p-6 sm:p-8">
          ...
          <p className="mt-4 text-sm leading-6 text-black/55">{pulse.intent || 'People are changing this post one move at a time.'}</p>
          <div className="mt-5 flex flex-wrap items-center gap-4 text-xs font-semibold text-black/50"><span className="inline-flex items-center gap-1.5"><Users size={14}/>{participantCount(ordered)} changed</span><span className="inline-flex items-center gap-1.5"><Zap size={13}/>{ordered.length} moves</span></div>
        </div>
      </motion.article>

      <div className="mt-5 rounded-[22px] border border-black/10 bg-white/70 px-5 py-4 text-sm font-medium">{pulse.status === 'active' ? 'This post is still moving. You can change what happens next.' : 'This Pulse is complete. See how it changed.'}</div>
      ...
      {pulse.status === 'active' && !joined && <div className="fixed inset-x-0 bottom-0 z-20 border-t border-black/10 bg-[#f4f1e9]/95 p-3 backdrop-blur-xl"><div className="mx-auto flex max-w-[760px] items-center justify-between gap-3"><div><div className="text-sm font-semibold">Change this Pulse.</div><div className="text-[11px] text-black/45">Your move becomes the next person's context.</div></div><button onClick={() => router.push(`/?pulse=${pulse.id}`)} className="flex items-center gap-2 rounded-full bg-black px-5 py-3 text-xs font-bold text-white">Make a move <ArrowRight size={14}/></button></div></div>}
    </div>
  </main>;
}
